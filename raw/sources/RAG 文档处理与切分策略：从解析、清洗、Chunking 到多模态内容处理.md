...
created: 2026-06-19
description: "深入解析 RAG 文档进入索引前的完整链路，涵盖文件解析、清洗、结构化、Chunking 策略、语义丢失处理、分层校验与多模态内容处理等工程化实践。重点围绕 文档从上传到入库要经过哪些环节？、如何选择合适的 Chunking 策略？、什么是语义丢失，为什么会发生？、如何处理结构丢失问题？ 等内容展开。"
tags:
  - "rag"
  - "document-processing"
  - "chunking"
...
还有个成本问题：它需要额外的 embedding 调用来计算句子相似度，文档量一大，账单就很可观。实际测试下来，语义切分的性能对阈值和最小块大小参数极为敏感。设置合理的 min\_chunk\_size（如 200-400 Token）可以避免超小片段问题，调优后效果会好很多。
...
还有个更工程化的方案是多向量索引（Multi-Vector Retriever），这是 LangChain 主推的做法：先用 MLLM 生成图片的结构化摘要（如"This is a flowchart showing the order processing pipeline..."），摘要入文本向量索引，原图存在 docstore 里。检索时先命中摘要，再通过 doc\_id 关联拉取原图，把原图 base64 编码后一起塞给多模态 LLM 生成。
