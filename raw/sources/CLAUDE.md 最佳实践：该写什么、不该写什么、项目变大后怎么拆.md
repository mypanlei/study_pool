...
|  | [CLAUDE.md](http://claude.md/) | [AGENTS.md](http://agents.md/) |
| --- | --- | --- |
...
简单说：通常来看 **[AGENTS.md](http://agents.md/)** 是跨工具标准， **[CLAUDE.md](http://claude.md/)** 是 Claude Code 专属入口。两者可以复用同一份基础指令。
...
|  | [CLAUDE.md](http://claude.md/) | `.claude/rules/` |
| --- | --- | --- |
...
|  | [CLAUDE.md](http://claude.md/) | [SPEC.md](http://spec.md/) |
| --- | --- | --- |
...
一句话简单理解就是： **[CLAUDE.md](http://claude.md/) 管长期行为规范，Spec 管当次任务约束。**

### 一句话总结

- **[CLAUDE.md](http://claude.md/)** ：Claude Code 专属的行为规范；根目录/用户级通常在会话开始时加载，子目录规则按需生效。
- **[AGENTS.md](http://agents.md/)** ：跨工具通用的“怎么干”规则，可被 `CLAUDE.md` 导入复用。
- **`.claude/rules/`** ：局部规则目录；不带 `paths` 更像全局规则，带 `paths` 才会在处理匹配文件时生效。
- **[SPEC.md](http://spec.md/)** ：需求规格文件，定义这次做什么，属于 Spec Coding 流程中的一环。
...
提交信息格式（如 `feat(scope): message` ）、分支命名规范、环境变量依赖。这些信息 Claude 从代码里读不出来，但一个新入职的工程师一定会问——把 [CLAUDE.md](http://claude.md/) 当成给新人写的 onboarding 文档来写就对了。

**5\. 当前任务的关键信息。**  
有时候 [CLAUDE.md](http://claude.md/) 不仅仅是“静态规范”，也可以作为“动态工作台”，来明确当前需要完成的任务和优先级，这通常要和静态规范的文件分离开（其实也不一定以 [CLAUDE.md](http://claude.md/) 命名，你叫 [AGENTS.md](http://agents.md/) 还是其他的都可以）。主要写：任务描述、验收标准、优先级、截止时间、依赖关系、阻塞问题、技术实现要点等关键信息。你可以将其作为 Agent 的持久化任务手册，这样即使跨会话也不会忘了该做什么。
...
这里有一个来自 OpenAI 的经验：能用工具强制执行的规则，不要写成自然语言。 [CLAUDE.md](http://claude.md/) 是软约束，Linter/Hook/CI 才是硬约束。一条规则如果没法机械化验证，Agent 迟早会偏离。
...
**起步：一份文件，几行核心规则。** 大部分中小项目停在这里就够了。关键是保持精简——我自己的 [CLAUDE.md](http://claude.md/) 很少超过 50 行。

**拆分：主文件做路由，规则分文件管理。** 根目录的 [CLAUDE.md](http://claude.md/) 只放项目概述和常用命令，架构规范、API 约定、测试要求分别放在独立文件里，用 `@path/to/file` 引用：
