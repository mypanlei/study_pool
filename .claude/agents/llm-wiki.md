...
### 5. Loop（自动化循环）— Loop Engineering 模式

**Loop 1: 自动采集** — 检查 `Clippings/` 是否有新文件，如有则搬入 `raw/sources/` 并执行 Ingest

**Loop 2: 健康检查** — 每日自动 Lint（孤儿页/索引一致性/源一致性），记录到 `wiki/log.md`

**Loop 3: 知识演进** — 扫描知识缺口，主动建议补充方向

**构建四步法**：
1. 窄任务开始 — 先跑通一个简单的 Loop（如 Lint）
2. 明确验证方式 — 如何确认 Loop 工作正常
3. 设置保险机制 — 不自动修改用户未确认的内容
